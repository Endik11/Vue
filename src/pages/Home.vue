...
const addToFavorite = async (item) => {
  try {
    if (!item.isFavorite) {
      ...
      const { data } = await axios.post(`https://12bbee3f13e60c22.mokky.dev/favorites`, obj) // Отправляем POST-запрос для добавления элемента в избранное
      ...
      await axios.delete(`https://12bbee3f13e60c22.mokky.dev/favorites/${item.favoriteId}`) // Отправляем DELETE-запрос для удаления элемента из избранного
...
const fetchFavorites = async () => {
  try {
    ...
    const { data: favorites } = await axios.get(`https://12bbee3f13e60c22.mokky.dev/favorites`)
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get(`https://12bbee3f13e60c22.mokky.dev/items`, { params })
